...
void Game::Loop()
{
    while (wnd->isOpen())
    {
        if (controlBody->GetLinearVelocity().y == 0.0f && controlBody->GetLinearVelocity().x == 0.0f) {
            controlBody->SetTransform(b2Vec2(10.0f, 90.0f), 0.0f);
        }
...
void Game::DoEvents()
{
    ...
    while (wnd->pollEvent(evt))
    {
        ...
        case Event::MouseButtonPressed:
            ...
            //controlBody->SetTransform(b2Vec2(pos.x, pos.y), 0.0f);
            ...
            controlBody->ApplyLinearImpulseToCenter(impulso, true);
            break;
        }
    }
...
void Game::InitPhysics()
{
    ...
    vec_cañon = new b2Vec2(50.0, 50.0);
    ...
    controlBody = Box2DHelper::CreateRectangularDynamicBody(phyWorld, 5.0, 5.0f, 5.0f, 1.0f, 0.0f);
    controlBody->SetTransform(b2Vec2(10.0f, 90.0f), 0.0f);
